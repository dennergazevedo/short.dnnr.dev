...
const sites = [
  ...
  },
  {
    ...
    name: 'os.dnnr.dev',
    description: 'Janelas XD — inspired by Windows XP',
    icon: '🖥️',
    bg: 'bg-neutral-100',
    ...
    <main className="flex min-h-screen flex-col">
        ...
        <HomeAnimations>
          <div className="relative text-center">
            ...
            <p className="mx-auto mt-5 max-w-md text-base text-zinc-500 sm:text-lg">
              Transforme URLs longas em links curtos, elegantes e rastreáveis — em segundos.
            </p>
          </div>
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {sites.map((site) => (
              ...
              >
                <div className={`mb-3 inline-flex h-9 w-9 items-center justify-center rounded-xl text-lg ${site.iconBg}`}>
                  {site.icon}
                </div>
                <p className={`font-mono text-sm font-semibold ${site.text}`}>{site.name}</p>
                <p className={`mt-0.5 text-xs ${site.sub}`}>{site.description}</p>
                <span className={`absolute right-4 top-4 text-lg font-light transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5 ${site.arrow}`}>
                  ↗
                </span>
              </a>
            ))}
      ...
      <footer className="border-t border-zinc-200 bg-white px-6 py-5">
        <div className="mx-auto flex max-w-5xl items-center justify-between text-xs text-zinc-400">
          ...
          <span>
            by{' '}
            <a href="https://dnnr.dev" className="hover:text-zinc-700 transition-colors">
              dnnr
            </a>
          </span>
        </div>
